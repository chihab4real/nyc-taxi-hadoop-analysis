...
if len(sys.argv) < 2:
    print("Usage: python precompute_plots.py <data_parquet_filename>")
    sys.exit(1)

data_file = sys.argv[1]
# Extract the base filename without extension for folder name
base_name = os.path.splitext(os.path.basename(data_file))[0]
base_dir = os.path.join("./saved_plots", base_name)
os.makedirs(base_dir, exist_ok=True)

print("Starting Spark session...")
spark = SparkSession.builder.appName("NYC Taxi Precompute").getOrCreate()

print(f"Loading data from {data_file}...")
df = spark.read.parquet(f"hdfs://namenode:8020/nyc_taxi/{data_file}")
...
# 1) Hourly Distribution of Taxi Trips
...
fig2 = px.bar(trip_day_df, x='day_of_week', y='trip_count',
              title='Taxi Trip Count by Day of Week',
...
# 3) Heatmap of Average Fare by Hour and Day
...
avg_revenue_df['day_of_week'] = pd.Categorical(avg_revenue_df['day_of_week'], categories=weekday_order, ordered=True)
...
fig3.update_layout(
    title="Interactive Heatmap of Average Fare by Hour and Day",
...
# 4) Tip Distribution Across Payment Methods
...
fig4 = px.box(
    ...
    x="payment_method",
    y="tip_amount",
    title="Tip Amount Distribution by Payment Type",
...
# 5) Hourly Average Speed of Taxis
...
fig5 = px.line(speed_hour_df, x="pickup_hour", y="avg_speed_kmph",
              title="Average Taxi Speed by Hour of Day", markers=True)


fig5.write_html(os.path.join(base_dir, "hourly_avg_speed.html"))

# 6) Folium map of pickups and dropoffs
...
import json
import builtins  # Ensure this is at the top of your script if not already
...
spark.stop()
print(f"✅ All plots saved in folder: {base_dir}")
